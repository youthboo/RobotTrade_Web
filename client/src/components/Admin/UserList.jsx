...
function UserList() {
  ...
  const deleteUser = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`http://192.168.15.227:5656/api/users/${id}`);
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`http://192.168.15.227:5656/api/users/${editUserId}`, editFormData);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://192.168.15.227:5656/api/users');
