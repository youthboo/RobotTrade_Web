...
function UserPayment() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.15.227:5656/api/get-payment?email=${email}`, {
        method: 'GET',
        headers: {
      ...
      const paymentIntentResponse = await fetch('http://192.168.15.227:5656/api/create-payment-intent', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
